...
def detect_ports():
    """
    Tüm USB seri portlarını tarar ve cihaz yanıtlarına göre otomatik eşleştirir.
    - GRBL Arduino: Bağlanınca 'Grbl' banner'ı gönderir
    - Nozzle Arduino: 'SLAVE_READY' veya PING→PONG yanıtı verir
    
    Sonuç global detected_ports dict'ine yazılır.
    """
    import serial
    ...
    if not ports:
        log.warning("Hiç USB seri port bulunamadı!")
        return detected_ports

    log.info(f"Port tarama başlatıldı: {', '.join(ports)}")

    for p in ports:
        try:
            log.info(f"  Port test ediliyor: {p}")
            s = serial.Serial(p, 115200, timeout=2)
            s.dtr = False
            time.sleep(0.5)
            s.flushInput()
            s.dtr = True
            time.sleep(3)  # Arduino reset + GRBL banner bekleme

            # Porttan gelen tüm yanıtı oku
            resp = ""
            t_start = time.time()
            ...
            if 'grbl' in resp.lower():
                detected_ports["grbl"] = p
                s.close()
                log.info(f"  ✓ GRBL bulundu: {p}")
                continue

            # ── Nozzle Arduino Kontrolü ──
            # Startup mesajında SLAVE_READY varsa
            if 'slave_ready' in resp.lower():
                detected_ports["nozzle"] = p
                s.close()
                log.info(f"  ✓ Nozzle Arduino bulundu (SLAVE_READY): {p}")
                continue

            # PING komutu gönder ve PONG yanıtını kontrol et
            try:
                ...
                while time.time() - t2 < 2:
                    if s.in_waiting:
                        ping_resp += s.read(s.in_waiting).decode('utf-8', errors='ignore')
                    time.sleep(0.1)

                if 'pong' in ping_resp.lower() or 'slave_ready' in ping_resp.lower():
                    detected_ports["nozzle"] = p
                    s.close()
                    log.info(f"  ✓ Nozzle Arduino bulundu (PING/PONG): {p}")
                    continue
            except Exception:
                pass

            s.close()
            log.info(f"  ? Tanımlanamayan cihaz: {p}")

        except Exception as e:
            log.warning(f"  ✗ Port açılamadı ({p}): {e}")
            continue

    # Sonuç özeti
    grbl_port = detected_ports.get("grbl", "Bulunamadı")
    nozzle_port = detected_ports.get("nozzle", "Bulunamadı")
    log.info(f"Port tarama tamamlandı → GRBL: {grbl_port or 'Bulunamadı'} | Nozzle: {nozzle_port or 'Bulunamadı'}")
...
class PNPDriver:
    ...
    def find_port(self):
        ...
        Önce detect_ports() sonucunu kullanır."""
        global detected_ports
        if detected_ports.get("grbl"):
            log.info(f"GRBL portu (otomatik algılama): {detected_ports['grbl']}")
            return detected_ports["grbl"]

        # Fallback: eski tarama yöntemi
        import serial
        ports = sorted(glob.glob('/dev/ttyUSB*') + glob.glob('/dev/ttyACM*'))
        if not ports:
            return None

        log.info(f"Taranıyor (fallback): {', '.join(ports)}")
        for p in ports:
            try:
                s = serial.Serial(p, 115200, timeout=2)
                s.dtr = False
...
class NozzleController:
    ...
    def connect(self, port=None):
        """Slave Arduino'ya bağlan. Port otomatik algılanır."""
        ...
        with self._lock:
            ...
            # Port seçim önceliği:
            # 1. Fonksiyona verilen port parametresi
            # 2. detect_ports() ile bulunan nozzle portu
            # 3. Config'deki varsayılan port (/dev/arduino_slave)
            if not port:
                global detected_ports
                if detected_ports.get("nozzle"):
                    port = detected_ports["nozzle"]
                    log.info(f"Nozzle portu (otomatik algılama): {port}")
                elif os.path.exists(config.NOZZLE_SERIAL_PORT):
                    port = config.NOZZLE_SERIAL_PORT
                else:
                    # Son çare: GRBL olmayan ilk portu dene
                    import glob as _glob
                    all_ports = sorted(_glob.glob('/dev/ttyUSB*') + _glob.glob('/dev/ttyACM*'))
                    grbl_port = detected_ports.get("grbl")
                    remaining = [p for p in all_ports if p != grbl_port]
                    if remaining:
                        port = remaining[0]
                        log.info(f"Nozzle portu (kalan port): {port}")
                    else:
                        return False, "Nozzle portu bulunamadı"
...
def _run_sub_steps(steps, pnp_ref, camera_ref, socketio_ref, emit_fn, parent_label=""):
    """
    Alt adımları çalıştır (IF blokları ve pick_and_test için).
    run_scenario içindeki adım motorunun aynısını kullanır.
    ...
    while si < len(steps):
        ...
        stype = step.get('type')
        label = f"{parent_label}[{si+1}/{len(steps)}]"
        emit_fn('running', f"{label} {_step_description(step)}")
        ...
        if stype == 'goto_base':
            ...
            if target:
                ...
                emit_fn('warning', f"{label} Konum bulunamadı: {base_name}")

        elif stype == 'auto_center':
            word = step.get('word', '')
            if word:
                emit_fn('running', f"{label} '{word}' kelimesine merkezleniyor...")
                auto_center(camera_ref, pnp_ref, socketio_ref, word)
            else:
                emit_fn('warning', f"{label} Merkezleme kelimesi boş!")

        elif stype == 'pump_on':
            pnp_ref.pump(True)
            emit_fn('running', f"{label} Pompa açıldı.")

        elif stype == 'pump_off':
            pnp_ref.pump(False)
            emit_fn('running', f"{label} Pompa kapatıldı.")

        elif stype == 'delay':
            secs = float(step.get('seconds', 1))
            emit_fn('running', f"{label} {secs}s bekleniyor...")
            waited = 0
            while waited < secs:
                if scenario_stop_flag:
                    return False
            ...
            pnp_ref.home()
            emit_fn('running', f"{label} GRBL Home'a gidildi.")
            time.sleep(0.5)
            socketio_ref.emit('motor_update', pnp_ref.get_status())
            ...
            emit_fn('running', f"{label} Z ekseni {z_val}mm konumuna gidiliyor...")
            pnp_ref.move_absolute(z=z_val)
            socketio_ref.emit('motor_update', pnp_ref.get_status())

        elif stype == 'verify':
            emit_fn('running', f"{label} 👁️ Doğruluk Kontrolü yapılıyor...")
        ...
        # ── TEK SEFERLIK DİRENÇ ÖLÇÜMÜ (Ana sayfa butonu gibi) ────────
        elif stype == 'resistance_read':
            emit_fn('running', f"{label} 🔬 Direnç Ölçülüyor...")
            if nozzle.connected:
                result = nozzle.read_resistance()
                if result.get('success'):
                    r_str = result.get('resistance_formatted', '?')
                    emit_fn('running', f"{label} 🔬 Direnç: {r_str} (ADC:{result.get('adc',0)} V:{result.get('voltage',0)}V)")
                    socketio_ref.emit('scenario_test_result', {
                        'type': 'resistance',
                        'measured': result.get('resistance', 0),
                        'measured_formatted': r_str,
                        'decision': r_str
                    })
                else:
                    emit_fn('warning', f"{label} Direnç ölçümü başarısız!")
            else:
                emit_fn('warning', f"{label} Nozzle bağlı değil! Direnç ölçümü atlandı.")

        # ── TEK SEFERLIK DİYOT TESTİ (Ana sayfa butonu gibi) ──────────
        elif stype == 'diode_read':
            emit_fn('running', f"{label} 💡 Diyot Test Ediliyor...")
            if nozzle.connected:
                result = nozzle.read_diode()
                if result.get('success'):
                    passing = result.get('current_passing', False)
                    status_str = "AKIM GEÇİYOR ✅" if passing else "AKIM GEÇMİYOR ❌"
                    emit_fn('running', f"{label} 💡 Diyot: {status_str} (ADC:{result.get('adc',0)} Eşik:{result.get('threshold',0)})")
                    socketio_ref.emit('scenario_test_result', {
                        'type': 'diode',
                        'actual_passing': passing,
                        'decision': status_str
                    })
                else:
                    emit_fn('warning', f"{label} Diyot testi başarısız!")
            else:
                emit_fn('warning', f"{label} Nozzle bağlı değil! Diyot testi atlandı.")

        elif stype == 'resistance_test':
            emit_fn('running', f"{label} 🔬 Direnç Testi yapılıyor...")
            if nozzle.connected:
                count = 1
                interval = 0.0
                ...
                emit_fn('running', f"{label} 🔬 Direnç: {avg_str} ({result.get('valid_count', 0)}/{count} geçerli)")
                socketio_ref.emit('scenario_test_result', {
                    'type': 'resistance',
                    'measured': result.get('average', 0),
                    'measured_formatted': avg_str,
                    'decision': avg_str
                })
            else:
                emit_fn('warning', f"{label} Nozzle bağlı değil! Direnç testi atlandı.")

        elif stype == 'diode_test':
            emit_fn('running', f"{label} 💡 Diyot Testi yapılıyor...")
            if nozzle.connected:
                count = 1
                interval = 0.0
                ...
                emit_fn('running', f"{label} 💡 Diyot: {decision}{corrected}")
                socketio_ref.emit('scenario_test_result', {
                    'type': 'diode',
                    'actual_passing': result.get('current_passing', False),
                    'decision': decision
                })
            else:
                emit_fn('warning', f"{label} Nozzle bağlı değil! Diyot testi atlandı.")
            ...
            emit_fn('running', f"{label} 🔄 Nozzle {angle}° açıya döndürülüyor...")
            if nozzle.connected:
                moved, new_pos, msg = nozzle.goto_angle(angle)
                emit_fn('running', f"{label} 🔄 Nozzle: {msg} | Pozisyon: {new_pos:.1f}°")
                socketio_ref.emit('nozzle_status', nozzle.get_status())
            else:
                emit_fn('warning', f"{label} Nozzle bağlı değil!")

        elif stype == 'nozzle_home':
            emit_fn('running', f"{label} 🏠 Nozzle Homing yapılıyor...")
            if nozzle.connected:
                success, msg = nozzle.home()
                emit_fn('running', f"{label} 🏠 Nozzle: {msg}")
                socketio_ref.emit('nozzle_status', nozzle.get_status())
            else:
                emit_fn('warning', f"{label} Nozzle bağlı değil!")
        ...
        # ── IF RESISTANCE ─────────────────────────────────────────────────
            ...
            tolerance_pct = float(step.get('tolerance_percent', 10))
            test_count = int(step.get('test_count', config.NOZZLE_TEST_COUNT))
            pass_steps = step.get('pass_steps', [])
            fail_steps = step.get('fail_steps', [])

            emit_fn('running', f"{label} 🔬 IF Direnç: hedef={target_r}Ω ±{tolerance_pct}%")

            if not nozzle.connected:
                emit_fn('warning', f"{label} Nozzle bağlı değil! IF blok atlandı.")
            else:
                test_count = 1
                ...
                lower = target_r * (1 - tolerance_pct / 100.0)
                upper = target_r * (1 + tolerance_pct / 100.0)
                in_range = (lower <= avg_ohm <= upper)

                status_icon = "✅ PASS" if in_range else "❌ FAIL"
                emit_fn('running',
                    f"{label} 🔬 Direnç: {avg_str} | Hedef: {target_r}Ω ±{tolerance_pct}% "
                    f"[{lower:.0f}-{upper:.0f}Ω] → {status_icon}")

                # SocketIO ile detaylı bildirim
                socketio_ref.emit('scenario_test_result', {
                    'type': 'resistance',
                    'measured': avg_ohm,
                    'measured_formatted': avg_str,
                ...
                if in_range:
                    if pass_steps:
                        emit_fn('running', f"{label} ✅ Aralıkta → PASS adımları çalıştırılıyor...")
                        if not _run_sub_steps(pass_steps, pnp_ref, camera_ref, socketio_ref, emit_fn, f"{label}P"):
                            return False
                else:
                    if fail_steps:
                        emit_fn('running', f"{label} ❌ Aralık dışı → FAIL adımları çalıştırılıyor...")
                        if not _run_sub_steps(fail_steps, pnp_ref, camera_ref, socketio_ref, emit_fn, f"{label}F"):
                            return False

        # ── IF DIODE ──────────────────────────────────────────────────────
        elif stype == 'if_diode':
            expected = step.get('expected_passing', True)
            test_count = int(step.get('test_count', config.NOZZLE_TEST_COUNT))
            pass_steps = step.get('pass_steps', [])
            fail_steps = step.get('fail_steps', [])

            exp_str = "Akım Geçiyor" if expected else "Akım Geçmiyor"
            emit_fn('running', f"{label} 💡 IF Diyot: beklenen={exp_str}")

            if not nozzle.connected:
                emit_fn('warning', f"{label} Nozzle bağlı değil! IF blok atlandı.")
            else:
                test_count = 1
                result = nozzle.diode_test_multi(
                    count=test_count,
                    interval=0.0,
                    auto_correct=False,   # otomatik düzeltme yapmadan ölç
                    socketio_ref=socketio_ref
                )
                actual_passing = result.get('current_passing', False)
                match = (actual_passing == expected)

                actual_str = "Akım Geçiyor" if actual_passing else "Akım Geçmiyor"
                status_icon = "✅ PASS" if match else "❌ FAIL"
                emit_fn('running',
                    f"{label} 💡 Diyot: {actual_str} | Beklenen: {exp_str} → {status_icon}")

                socketio_ref.emit('scenario_test_result', {
                    'type': 'diode',
                    'actual_passing': actual_passing,
                    'expected_passing': expected,
                    'match': match,
                ...
                if match:
                    if pass_steps:
                        emit_fn('running', f"{label} ✅ Beklenen sonuç → PASS adımları çalıştırılıyor...")
                        if not _run_sub_steps(pass_steps, pnp_ref, camera_ref, socketio_ref, emit_fn, f"{label}P"):
                            return False
                else:
                    if fail_steps:
                        emit_fn('running', f"{label} ❌ Farklı sonuç → FAIL adımları çalıştırılıyor...")
                        if not _run_sub_steps(fail_steps, pnp_ref, camera_ref, socketio_ref, emit_fn, f"{label}F"):
                            return False

        # ── PICK AND TEST ─────────────────────────────────────────────────
        elif stype == 'pick_and_test':
            comp_word = step.get('component_word', '')
            pick_base = step.get('pick_base', 'KOMPONENT BASE')
            z_pick = float(step.get('z_pick', -166))
            test_type = step.get('test_type', 'resistance')
            test_cfg = step.get('test_config', {})
            pass_place = step.get('pass_place_base', '')
            fail_place = step.get('fail_place_base', '')
            safe_base = step.get('safe_base', 'KOMPONENT BASE')
            z_place = float(step.get('z_place', -168))

            emit_fn('running', f"{label} 🤖 Al-Test-Yerleştir: {comp_word} ({test_type})")

            # 1. Güvenli konuma git
            _goto_base_safe(safe_base, pnp_ref, socketio_ref)
            time.sleep(0.3)

            # 2. Merkezleme
            if comp_word:
                emit_fn('running', f"{label} 🎯 '{comp_word}' merkezleniyor...")
                auto_center(camera_ref, pnp_ref, socketio_ref, comp_word)
                time.sleep(0.5)

            # 3. Pick (al)
            emit_fn('running', f"{label} ⬇️ Z indiriliyor ({z_pick}mm)...")
            pnp_ref.move_absolute(z=z_pick)
            time.sleep(0.3)
            pnp_ref.pump(True)
            emit_fn('running', f"{label} 💨 Pompa açıldı — komponent alındı.")
            time.sleep(1)

            # 4. Z kaldır ve güvenli konuma git
            pnp_ref.move_absolute(z=0)
            time.sleep(0.3)
            _goto_base_safe(safe_base, pnp_ref, socketio_ref)
            socketio_ref.emit('motor_update', pnp_ref.get_status())
            ...
            test_passed = False
            if not nozzle.connected:
                emit_fn('warning', f"{label} Nozzle bağlı değil! Test atlanıyor, PASS varsayılıyor.")
                test_passed = True
            elif test_type == 'resistance':
                target_r = float(test_cfg.get('target_resistance', 10000))
                tol_pct = float(test_cfg.get('tolerance_percent', 10))
                t_count = 1
                emit_fn('running', f"{label} 🔬 Direnç Testi: hedef={target_r}Ω ±{tol_pct}%")
                ...
                avg_ohm = result.get('average', 0)
                avg_str = result.get('average_formatted', '?')
                lower = target_r * (1 - tol_pct / 100.0)
                upper = target_r * (1 + tol_pct / 100.0)
                test_passed = (lower <= avg_ohm <= upper)

                status_icon = "✅ PASS" if test_passed else "❌ FAIL"
                emit_fn('running',
                    f"{label} 🔬 Direnç: {avg_str} [{lower:.0f}-{upper:.0f}Ω] → {status_icon}")
                ...
                socketio_ref.emit('scenario_test_result', {
                    ...
                    'in_range': test_passed,
                    'decision': status_icon
                })

            elif test_type == 'diode':
                expected = test_cfg.get('expected_passing', True)
                t_count = 1
                emit_fn('running', f"{label} 💡 Diyot Testi yapılıyor...")
                ...
                actual_passing = result.get('current_passing', False)
                test_passed = (actual_passing == expected)

                status_icon = "✅ PASS" if test_passed else "❌ FAIL"
                emit_fn('running', f"{label} 💡 Diyot → {status_icon}")
            ...
            # 6. Sonuca göre yerleştir
            place_base = pass_place if test_passed else fail_place
            if place_base:
                emit_fn('running', f"{label} 📍 '{place_base}' konumuna götürülüyor ({status_icon})...")
                _goto_base_safe(place_base, pnp_ref, socketio_ref)
                time.sleep(0.3)

                # 7. Place (yerleştir)
                # Hedef z değeri: base'in kendi z'si yoksa z_place kullan
                target_b = next((b for b in config.BASES if b['name'] == place_base), None)
                actual_z = target_b['z'] if target_b and target_b.get('z', 0) != 0 else z_place
                if pnp_ref.current_z != actual_z:
                    # Z zaten base goto ile ayarlanmış olabilir, tekrar kontrol et
                    pass
                pnp_ref.pump(False)
                emit_fn('running', f"{label} 🛑 Pompa kapatıldı — komponent yerleştirildi.")
                time.sleep(0.5)

                # 8. Z kaldır ve güvenli konuma dön
                pnp_ref.move_absolute(z=0)
                time.sleep(0.3)
                _goto_base_safe(safe_base, pnp_ref, socketio_ref)
            else:
                emit_fn('warning', f"{label} Yerleştirme konumu tanımlı değil!")
                pnp_ref.pump(False)
...
def _goto_base_safe(base_name, pnp_ref, socketio_ref):
    """Güvenli şekilde bir konuma git (Z sıralaması ile)."""
    target = next((b for b in config.BASES if b['name'] == base_name), None)
    if not target:
        log.warning(f"Konum bulunamadı: {base_name}")
        return
    current_z = pnp_ref.current_z
    target_z = target['z']
    if target_z < current_z:
    ...
    try:
        ...
        while i < len(steps):
            ...
            # ── TEK SEFERLIK DİRENÇ ÖLÇÜMÜ (Ana sayfa butonu gibi) ────
                ...
                emit('running', "🔬 Direnç Ölçülüyor...", i)
                if not nozzle.connected:
                    emit('warning', "Nozzle Arduino bağlı değil! Direnç ölçümü atlandı.", i)
                else:
                    result = nozzle.read_resistance()
                    if result.get('success'):
                        r_str = result.get('resistance_formatted', '?')
                        emit('running', f"🔬 Direnç: {r_str} (ADC:{result.get('adc',0)} V:{result.get('voltage',0)}V)", i)
                        socketio_ref.emit('scenario_test_result', {
                            'type': 'resistance',
                            'measured': result.get('resistance', 0),
                            'measured_formatted': r_str,
                        ...
                        emit('warning', "Direnç ölçümü başarısız!", i)

            # ── TEK SEFERLIK DİYOT TESTİ (Ana sayfa butonu gibi) ──────
            elif stype == 'diode_read':
                emit('running', "💡 Diyot Test Ediliyor...", i)
                if not nozzle.connected:
                    ...
                    if result.get('success'):
                        passing = result.get('current_passing', False)
                        status_str = "AKIM GEÇİYOR ✅" if passing else "AKIM GEÇMİYOR ❌"
                        emit('running', f"💡 Diyot: {status_str} (ADC:{result.get('adc',0)} Eşik:{result.get('threshold',0)})", i)
                        socketio_ref.emit('scenario_test_result', {
                            'type': 'diode',
                            'actual_passing': passing,
                            'decision': status_str
                        ...
                        emit('warning', "Diyot testi başarısız!", i)

            elif stype == 'resistance_test':
                emit('running', "🔬 Direnç Testi yapılıyor...", i)
                if not nozzle.connected:
                ...
                emit('running', "💡 Diyot Testi yapılıyor...", i)
                if not nozzle.connected:
                    emit('warning', "Nozzle Arduino bağlı değil! Diyot testi atlandı.", i)
                else:
                    count = 1
            ...
            # ── IF RESISTANCE (ana senaryo) ────────────────────────────────
                ...
                emit('running', f"🔬 IF Direnç: hedef={target_r}Ω ±{tolerance_pct}%", i)

                if not nozzle.connected:
                    emit('warning', "Nozzle bağlı değil! IF blok atlandı.", i)
                else:
                    test_count = 1
                    result = nozzle.resistance_test_multi(
                        count=test_count,
                    ...
                    status_icon = "✅ PASS" if in_range else "❌ FAIL"
                    emit('running',
                        f"🔬 Direnç: {avg_str} | Hedef: {target_r}Ω ±{tolerance_pct}% "
                        f"[{lower:.0f}-{upper:.0f}Ω] → {status_icon}", i)

                    socketio_ref.emit('scenario_test_result', {
                        'type': 'resistance', 'measured': avg_ohm,
                        'measured_formatted': avg_str, 'target': target_r,
                        'tolerance_percent': tolerance_pct,
                        'lower_bound': lower, 'upper_bound': upper,
                        'in_range': in_range, 'decision': status_icon
                    })

                    def emit_sub(status, message, step_index=-1):
                        socketio_ref.emit('scenario_update', {
                            'status': status, 'message': message,
                            'step_index': i, 'total_steps': len(steps),
                            'step_type': 'if_resistance', 'scenario_name': name
                        })

                    if in_range and pass_steps:
                        emit('running', f"✅ Aralıkta → PASS adımları çalıştırılıyor...", i)
                        _run_sub_steps(pass_steps, pnp_ref, camera_ref, socketio_ref, emit_sub, f"[{i+1}]P")
                    elif not in_range and fail_steps:
                        emit('running', f"❌ Aralık dışı → FAIL adımları çalıştırılıyor...", i)
                        _run_sub_steps(fail_steps, pnp_ref, camera_ref, socketio_ref, emit_sub, f"[{i+1}]F")

            # ── IF DIODE (ana senaryo) ────────────────────────────────────
            elif stype == 'if_diode':
                expected = step.get('expected_passing', True)
                test_count = int(step.get('test_count', config.NOZZLE_TEST_COUNT))
                pass_steps = step.get('pass_steps', [])
                ...
                exp_str = "Akım Geçiyor" if expected else "Akım Geçmiyor"
                emit('running', f"💡 IF Diyot: beklenen={exp_str}", i)
                ...
                if not nozzle.connected:
                    ...
                    status_icon = "✅ PASS" if match else "❌ FAIL"
                    emit('running',
                        f"💡 Diyot: {actual_str} | Beklenen: {exp_str} → {status_icon}", i)

                    socketio_ref.emit('scenario_test_result', {
                        'type': 'diode', 'actual_passing': actual_passing,
                        'expected_passing': expected, 'match': match,
                        'decision': status_icon
                    })

                    def emit_sub_d(status, message, step_index=-1):
                        socketio_ref.emit('scenario_update', {
                            'status': status, 'message': message,
                            'step_index': i, 'total_steps': len(steps),
                            'step_type': 'if_diode', 'scenario_name': name
                        })

                    if match and pass_steps:
                        emit('running', f"✅ Beklenen sonuç → PASS adımları çalıştırılıyor...", i)
                        _run_sub_steps(pass_steps, pnp_ref, camera_ref, socketio_ref, emit_sub_d, f"[{i+1}]P")
                    elif not match and fail_steps:
                        emit('running', f"❌ Farklı sonuç → FAIL adımları çalıştırılıyor...", i)
                        _run_sub_steps(fail_steps, pnp_ref, camera_ref, socketio_ref, emit_sub_d, f"[{i+1}]F")

            # ── PICK AND TEST (ana senaryo) ───────────────────────────────
            elif stype == 'pick_and_test':
                comp_word = step.get('component_word', '')
                pick_base = step.get('pick_base', 'KOMPONENT BASE')
                z_pick = float(step.get('z_pick', -166))
                ...
                emit('running', f"🤖 Al-Test-Yerleştir: {comp_word} ({test_type})", i)
                ...
                # 2. Merkezleme
                if comp_word:
                    emit('running', f"🎯 '{comp_word}' merkezleniyor...", i)
                    auto_center(camera_ref, pnp_ref, socketio_ref, comp_word)
                    time.sleep(0.5)

                # 3. Pick
                emit('running', f"⬇️ Z indiriliyor ({z_pick}mm)...", i)
                pnp_ref.move_absolute(z=z_pick)
                time.sleep(0.3)
                pnp_ref.pump(True)
                emit('running', "💨 Pompa açıldı — komponent alındı.", i)
                time.sleep(1)
                ...
                status_icon = "❓"
                if not nozzle.connected:
                    emit('warning', "Nozzle bağlı değil! Test atlanıyor, PASS varsayılıyor.", i)
                    test_passed = True
                    status_icon = "⚠️ SKIP"
                elif test_type == 'resistance':
                    target_r = float(test_cfg.get('target_resistance', 10000))
                    tol_pct = float(test_cfg.get('tolerance_percent', 10))
                    t_count = 1
                    emit('running', f"🔬 Direnç Testi: hedef={target_r}Ω ±{tol_pct}%", i)
                    result = nozzle.resistance_test_multi(
                        count=t_count,
                        interval=0.0,
                        socketio_ref=socketio_ref
                    ...
                    test_passed = (lower <= avg_ohm <= upper)
                    status_icon = "✅ PASS" if test_passed else "❌ FAIL"
                    emit('running', f"🔬 Direnç: {avg_str} [{lower:.0f}-{upper:.0f}Ω] → {status_icon}", i)
                    socketio_ref.emit('scenario_test_result', {
                        'type': 'resistance', 'measured': avg_ohm,
                        'measured_formatted': avg_str, 'target': target_r,
                        'in_range': test_passed, 'decision': status_icon
                    })
                elif test_type == 'diode':
                    expected = test_cfg.get('expected_passing', True)
                    t_count = 1
                    ...
                    result = nozzle.diode_test_multi(
                        count=t_count,
                        interval=0.0,
                        auto_correct=False, socketio_ref=socketio_ref
                    )
                    actual_passing = result.get('current_passing', False)
                    test_passed = (actual_passing == expected)
                    status_icon = "✅ PASS" if test_passed else "❌ FAIL"
                    emit('running', f"💡 Diyot → {status_icon}", i)
                    socketio_ref.emit('scenario_test_result', {
                        'type': 'diode', 'actual_passing': actual_passing,
                        'expected_passing': expected, 'match': test_passed,
                        'decision': status_icon
                    })
                ...
                place_base = pass_place if test_passed else fail_place
                if place_base:
                    emit('running', f"📍 '{place_base}' konumuna götürülüyor ({status_icon})...", i)
                    _goto_base_safe(place_base, pnp_ref, socketio_ref)
                    time.sleep(0.3)
                    pnp_ref.pump(False)
                    emit('running', "🛑 Pompa kapatıldı — komponent yerleştirildi.", i)
                    time.sleep(0.5)
                    pnp_ref.move_absolute(z=0)
                    time.sleep(0.3)
                    _goto_base_safe(safe_base, pnp_ref, socketio_ref)
                else:
                    emit('warning', "Yerleştirme konumu tanımlı değil!", i)
                    pnp_ref.pump(False)
...
def _step_description(step):
    ...
    if t == 'if_resistance':
        return f"🔬 IF Direnç: {step.get('target_resistance', 0)}Ω ±{step.get('tolerance_percent', 10)}% → [{len(step.get('pass_steps', []))} pass / {len(step.get('fail_steps', []))} fail adım]"
    if t == 'if_diode':
        exp = "Geçiyor" if step.get('expected_passing', True) else "Geçmiyor"
...
def main():
    ...
    # 2. Tüm portları tara — GRBL ve Nozzle'ı otomatik bul
    detect_ports()

    # 3. PNP motoruna bağlan (algılanan GRBL portunu kullanır)
    try:
        pnp.connect()
    ...
    # 4. Nozzle Arduino'ya bağlan (algılanan Nozzle portunu kullanır)
    try:
